...
\title{Byzantine Fault Allowing: A Category of Fault-Tolerant State Machine Replica Protocols}
...
For the sake of exposition, consider first several embeddings of the data of $B$ into a space which may behave as a suitable metric space.

By our initial definition of consistency $Con$, quora are indexed by the natural numbers $o: \mathbb(N) < |Q| \twoheadrightarrow Q$. 

To construct our initial projection, we will assume a bijection between the natural numbers and the replicas $p: \mathbb(N) < |P| \twoheadrightarrow P$.

And, finally, we will assume a bijection between the natural numbers and the values $r' \in R$ $r: \mathbb(N) \twoheadrightarrow R$.

We may naively project this into $\mathbb{R}^3$ by the identity inclusion $i(n) = n$ for each datum:

\begin{flalign*}
\phi_{r,o,p}: Q \to \mathbb{R}^3 = (i(r), i(o), i(p))
\end{flalign*}
...
\begin{flalign*}
\phi_{r,o,p}: Q \to \mathbb{R}^3 = (i(r), i(o), \frac{i(p)}{|P|})
\end{flalign*}
...
\begin{flalign*}
  \phi_{r,o,p,\cdot}: Q \to \mathbb{R}^3 = (i(r) \cdot i(o), i(r) \cdot \frac{i(p)}{|P|}, i(o) \cdot \frac{i(p)}{|P|})
\end{flalign*}
...
This correlates the all components of the projection with each other while preserving information about the nature of the formation of the pairwise interaction. 

Finally, we consider a Order-2 projection in which each $\phi_{r,o,p,T}: Q \to \mathbb{R}^{|P| \times 2}$ wherein each quorum index and value pair is inserted into the matrix at the index of the replica:

\begin{flalign*}
& \phi_{r,o,T}(q)*{j,1} = \mathbf{1}*{{\rho_j\in q}}, i(o_{\rho_j}),\qquad \\
& \phi_{r,o,T}(q)*{j,2} = \mathbf{1}*{{\rho_j\in q}},\frac{i_T(p_{\rho_j})}{|P|} \\
\end{flalign*}
...
\subsection{Combinatorial Morphisms and the Fixed Point of $B$}
...
Assume also an algorithm $A$ which produces quora $Q_B(r') \subseteq Q_A(r')$ is equivalent to $B$.
