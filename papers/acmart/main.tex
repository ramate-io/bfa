...
Consider the Byzantine Assumption, which is that for $3f + 1$ replicas, at most $f$ replicas are Byzantine. For any single state transition, $r \to r'$, this assumes that there are $2f + 1$ replicas which compute and broadcast a correct state transition. These replicas would be considered "perfect experts" w.r.t. $r \to r'$. 
...
\begin{align*}
|Q(r'_1) \cap Q(r'_2)| = |Q(r'_1)| + |Q(r'_2)| - |Q(r'_1) \cup Q(r'_2)| \\
...
\begin{align*}
|Q(r'_1) \cap Q(r'_2)| = |Q(r'_1) \cap Q(r'_2) \cap H| + |Q(r'_1) \cap Q(r'_2) \cap F| \\
Q(r'_1) \cap Q(r'_2) \cap H| = |Q(r'_1) \cap Q(r'_2)| - |Q(r'_1) \cap Q(r'_2) \cap F| \\
...
We now consider what it would mean for a participant $p \in P$ to be a perfect expert and what loss from this expert would mean. 

It is common to define a perfect expert as an expert which records the correct result at any given step $t_1, t_2, \cdots, t_n$. Given that, for any view of consensus, our results are recorded in quora, we argue that a perfect expert would be a member of all quora $p' \in Q(r') \forall r' \in R'$. This implies both correctness and liveness, which address both separately and jointly. 

By itself, correctness of a given replica for a single state transition $r' \in R'$ is simply whether it computes the same result as the honest replicas, i.e., whether the quorum of which it is a member intersects with the honest set $H$:

\begin{align*}
  Correct: P \times R' \to {0, 1} \\
  Correct(p, r') = p \in Q(r') \land Q(r') \cap H \neq \emptyset \\
\end{align*}

The liveness of a given replica for a single state transition $r' \in R'$ is more abstract. We argue for a particular framework of liveness in this paper's main theory. For simplicity, in this section, consider that quora mark units in time. We can measure a participant's liveness between any two quora as follows:

\begin{align*}
  Live: P \times R' \times R' \to {0, 1} \\
  Live(p, r_1', r_2') = p \in Q(r_1') \cap Q(r_2') \\
\end{align*}

For any two state transitions $r'_1, r'_2 \in R'$, we would like for our 

\begin{align*}
  p \in Q(r_1') \land Q(r_1') \cap H \neq \emptyset 
    \land p \in Q(r_2') \land Q(r_2') \cap H \neq \emptyset 
    \land p \in Q(r_1') \cap Q(r_2')
  Simplifies to:
  ...
  p \in Q(r_1') \cap Q(r_2') \cap H

\end{align*}

\begin{align*}
  Score: P \times R' \times R' \to {0, 1} \\
  Score(p, r_1', r_2') = \begin{cases}
    0 & p \in Q(r_1') \cap Q(r_2') \cap H \\
    1 & p \notin Q(r_1') \cap Q(r_2') \cap H \\
  \end{cases} \\
  = \begin{cases}
    0 & p \in Q(r_1') \cap Q(r_2') \cap H \\
    1 & p \notin Q(r_1') \cap Q(r_2') \cap H \\
...
Summing this score for all pairs of state transitions $r'_1, r'_2 \in R'$, we can compute the loss of the perfect expert to be $0$ as intended. 

\begin{align*}
  Loss: P \times 2^{R'} \to \mathbb{C} \\
  Loss(p, R') = \sum_{r'_1, r'_2 \in R'} Score(p, r'_1, r'_2) \\
  p'in Q(r'_1) \cap Q(r'_2) \cap H \implies Loss(p, R') = \\
  \sum_{r'_1, r'_2 \in R'} (Score(p', r'_1, r'_2) = 0) = 0 \\
\end{align*}

Under our Byzantine assumption, from Theorem 1, we have that any two quora must intersect in at least one honest replica. This does not generalize to all quora, so we do not have a single perfect expert. However, if we generalize our loss function to the quora itself, we can show that a BFT state machine replica protocol is a lossless expert model. 

\begin{proposition}
BFT state machine replicas are lossless expert models.
\end{proposition}

\begin{proof}
First we define the correctness component of our quorum score. Because the quorum agrees on $r'$ and any $h \in H$ correctly computes $r'$, if the quorum intersects with $r'$ then the quorum is correct.
