...
The probability of selecting a $\text{Corrupt}$ subcommittee $Pr[\mathcal{C'}](\gamma, k)$ tends towards 0 as $f$ increases for a fixed ratio $\gamma = \frac{k}{f}$.
...
Observe that the $Obj(Resample(M_i)) = 1$ if and only if we eventually select a $\text{Faulty}$ subcommittee. Because each sample step is independent, the probability of selecting a $\text{Faulty}$ subcommittee on the $n$th step is simply $Pr[\text{Faulty}]$ and terminating. This means that the probability of selecting a $\text{Faulty}$ subcommittee on the whole is the probability of selecting 0 $Right$ subcommittees during any of the $n$ steps.


Practically, this can be limited by a maximum number of resamples. But, we can also consider the expected number of resamples to render a decision.

The expected number of resamples to render a decision is given by:

\begin{flalign*}
  \mathbb{E}[\text{Resample Count}] = \sum_{n = 1}^{\infty} n \cdot Pr[\text{Resample Count} = n] \\
  &= \sum_{n = 1}^{\infty} n \cdot \left(1 - \frac{1}{2}\right)^{n-1} \cdot \frac{1}{2} \\
  &= \sum_{n = 1}^{\infty} n \cdot \frac{1}{2^n} \\
  &= 2 \\
\end{flalign*}

On average, we would expect $\mathbb{E}[Obj(Resample(M_i))] < 2 \cdot Pr[\text{Faulty}]$. Because $Pr[\text{Faulty}]$ tends towards 0 as $f$ increases and can be bounded, the likelihood of the protocol failing can be tuned quickly to be astronomically low.

We now show that $\text{Resample}$ is computationally efficient. Observe that $\text{Resample}$ uses at most $3k + 1$ processes on each sample step to render a decision. The expected number of steps to render a decision is:
...
For a fixed ratio $\gamma = \frac{k}{f} - 0.1$ we can reach cryptographically low probability of failure at fewer than 4096 processes. 
