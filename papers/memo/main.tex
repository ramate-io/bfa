...
To this end, I have chosen to align my views along two vantages. The first is the rich perspective of topological distributed computing. The second is the category theoretic consideration of relationships between Parabyzantine protocols. 
...
\]
we may form the \emph{nerve} simplicial complex $N(\mathcal{U})$ defined by:
\begin{flalign*}
...
When studying topologically, however, we may be able to show that $f \circ g$ and $g \circ f$ are equivalent up to homotopy:

\begin{flalign*}
  &f \circ g \simeq g \circ f \\
\end{flalign*}

In topology generally, we can access many of these non-standard equivalence relations. Homotopy, indistinguishability under a quotient space, and various more specific deformation relations, give us a lot of ways to relate two protocols which are not equivalent in the standard sense. As we used the average case to find the Parabyzantine \textsc{Resample} protocol, we may be able to infer other protocols which act like Byzantine fault-tolerant protocols up to some well-understood topological relationship and hence are Parabyzantine.

Thus, in addition to its extensive corpus, the topological formulation of Byzantine agreement is both a fitting tool for describing the sorts of properties we would like to preserve and source of instruments for relaxing requirements into Parabyzantine protocols.
...
A category of distributed protocols is a large undertaking. For the purposes of this memo, I will instead describe a small provisional construction which captures one dimension of protocol variation: the tradeoff between quorum cost and the strength of the order guarantee.
...
We will regard two protocol instances as equivalent when they induce the same quorum structure and the same induced order semantics at the level of this memo. In particular, when $k=f$ the resampling committee is the full population, so $\textsc{Resample}(f,f)$ is equivalent to the ``full quorum'' protocol $\mathcal{P}$.
...
To analogize, $\pb$ becomes a map of regions in Byzantine fault-tolerant computing that we know. Building bridges between these regions may reveal higher-order patterns which lead us to non-trivial constructions and better protocols.
