...
\begin{itemize}
  \item $|K_i \cap H| \geq 2k + 1$. This represents a subcommittee which has an honest supermajority which computes $M_i$ correctly. For ease of reference, we shall refer to this as kind of subcommittee as $\text{Right}$ and use the symbol $R$.
  \item $k < |K_i \cap H| < 2k + 1$. This represents a subcommittee that has neither an honest nor dishonest supermajority and may either compute $M_i$ correctly or disagree internally and not render a supermajority. For ease of reference, we shall refer to this as kind of subcommittee as $\text{Hung}$ and use the symbol $H$.
  \item $|K_i \cap H| \leq k$. This represents a subcommittee which has a dishonest supermajority and may compute $M_i$ incorrectly. For ease of reference, we shall refer to this as kind of subcommittee as $\text{Corrupt}$ and use the symbol $C$.
\end{itemize}
...
\begin{flalign*}
  ...
  &\text{Observe that } 1 - Pr[H] = Pr[R] + Pr[C] \implies \\
  &Pr[b] = \frac{Pr[C]}{Pr[R] + Pr[C]} \\
 \end{flalign*}

The probability of selecting a $\text{Right}$ subcommittee $Pr[R](f, k)$ tends downwards towards $\frac{1}{2}$ as $f$ increases for a fixed ratio $\gamma = \frac{k}{f}$.
...
\begin{flalign*}
  &\lim_{f \to \infty} Pr[H](\gamma, k) = \frac{1}{2} \forall k \in \mathbb{N}: k < f \\
\end{flalign*}
